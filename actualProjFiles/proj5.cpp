...
Animate( )
{
	...
	if (ANIMATE_FLG) {
		...
		ms %= ANIM_CYCLE;
		t = (float)ms / (float)(ANIM_CYCLE - 1000);
		if (t <= 1) {
			swapped = false;
			if (invs) {
				Time = t;
...
Display( )
{
	...
	Pattern->Use();
	if (animVert) {
		Pattern->SetUniformVariable("uVTime", Time);
	}
	else {
		Pattern->SetUniformVariable("uVTime", pauseTime);
	}

	Pattern->SetUniformVariable("uTime", Time);
	Pattern->SetUniformVariable("ux", AXES[0]);
	Pattern->SetUniformVariable("uy", AXES[1]);
	Pattern->SetUniformVariable("uz", AXES[2]);
	Pattern->SetUniformVariable("animFrag", animFrag);
