...
Animate( )
{
	...
	if (ANIMATE_FLG) {
		animateVar = 360 * 10 * Time;
		rotorAnim = 360 * 30 * Time;
	}
...
Display( )
{
	...
	if (partyMod) {
		gluLookAt(-.4, 1.8, -4.9, -.4, 1.8, -8., 0., 1., 0.);
	}
	else {
	...
	glScalef(5, 1, 1);
	glRotatef(90, 1, 0, 0);
	glCallList(RotorList);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(.5, 2.5, 9);
	glRotatef(rotorAnim, 1, 0, 0);
	glScalef(1.5, 1, 1);
	glRotatef(90, 0, 1, 0);
	glCallList(RotorList);
	glPopMatrix();
...
InitMenus( )
{
	...
	glutAddMenuEntry("Inside View", PARTY);
