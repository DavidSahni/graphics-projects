...
void InitTextures() {
	...
	int width, height;
	unsigned char* Texture = BmpToTexture("cool.bmp", &width, &height);
	printf("W H: %i, %i\n", width, height);
...
Animate( )
{
	...
	if (ANIMATE_FLG) {
		...
		int halfCycle = 4; 
		if (Time > .5) {
			animateVar = halfCycle * Time;
			animateVar = (halfCycle * .5) - (animateVar - (halfCycle * .5));
		}
		else {
			animateVar = halfCycle * Time;
		}
...
MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		
		{
			...
			{
				if (!woah) {
					float sS, sN, tS, tN, sO, tO;
					sS = (lng + M_PI) / (2.*M_PI);
					tS = (lat + M_PI / 2.) / M_PI;
					float nLat, nLong;
					nLat = ilat + 1;
					nLong = ilng + 1;
					nLat = -M_PI / 2. + M_PI * (float)(ilat + 1) / (float)(NumLats - 1);
					nLong = -M_PI + 2. * M_PI * (float)(ilng + 1) / (float)(NumLngs - 1); //get the next one
					sO = (nLong + M_PI) / (2.*M_PI) - sS;
					tO = (nLat + M_PI / 2.) / M_PI - tS;
					p->s = (sS + (sO*rev*animateVar));
					p->t = (tS + (sO*rev*animateVar));
				}
				else {
					float sS, sN, tS, tN, sO, tO;
					sS = (lng + M_PI) / (2.*M_PI);
					...
					nLat = ilat + 1;
					nLong = ilng + 1;
					sN = -M_PI / 2. + M_PI * (float)nLat / (float)(NumLats - 1);
					tN = -M_PI + 2. * M_PI * (float)nLong / (float)(NumLngs - 1); //get the next one
					sO = sN - sS;
					tO = tN - tS;
					p->s = (sS + (sO*rev*animateVar));
					p->t = (tS + (tO*rev*animateVar));
				}
