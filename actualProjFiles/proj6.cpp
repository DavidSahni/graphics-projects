...
const int DIST_WAVES = 4;
AngleC ANGS[DIST_WAVES * 10];
...
float	hexHeight;				// height of one hex trapezoid (for calculating translations)
bool	partyMod;
bool	ANIMATE_FLG;
float	animateVar;
float	rotorAnim;
...
void	Animate( );
void	Display( );
void	displayShine(float, float, float, bool);
void	DoAxesMenu( int );
...
void	drawBezier(Curve c);
void drawWaveyCurve(float , float , float, float, float, float);
void	updateAngs();
...
Animate( )
{
	...
	ms %= ANIM_CYCLE;
	float newTime = (float)ms / (float)(ANIM_CYCLE - 2000);
	if (newTime > 1) {
		Time = 1;
	}
...
void updateAngs() {
	for (int i = 0; i < DIST_WAVES * 10; i++) {
		float ang1 = (float)rand() / (float)(RAND_MAX / 180.);
		float ang2 = (float)rand() / (float)(RAND_MAX / 180.);

		ANGS[i].a1 = ang1;
		ANGS[i].a2 = ang2;
		ANGS[i].r = (float)rand() / (float)(RAND_MAX);
		ANGS[i].g = (float)rand() / (float)(RAND_MAX);
		ANGS[i].b = (float)rand() / (float)(RAND_MAX);
	}
}
...
Display()
{
	...
	float start = (DIST_WAVES / 2.) * -1;
	float end = (DIST_WAVES / 2.);
	int c = 0;
	for (float i = start; i < end; i+=.1) {

		drawWaveyCurve(ANGS[c].a1, ANGS[c].a2, ANGS[c].r, ANGS[c].g, ANGS[c].b, i);
		c++; //xd
...
void drawWaveyCurve(float deg1, float deg2, float r, float g, float b, float shift) {
	glPushMatrix();
...
void displayShine(float ang, float x, float y, bool border){
	for (int i = 0; i < 6; i++){
		glPushMatrix();
...
void genShineList() {
	...
	float hh = hThirdLen / 2.f;
	float hx = hh / 2;
	float hy = (hh / 2)*sqrt(3);
	hexHeight = hy;
	hy += .1;
