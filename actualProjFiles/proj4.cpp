...
Display( )
{
	...
	float xval = cos(degToRad(orbitVar)) * 3;
	float zval = sin(degToRad(orbitVar)) * 3;
	SetSpotLight(GL_LIGHT1, xval, .5, zval, -xval, 0, -zval, 102. / 255., 1, 102. / 255.);
	//SetSpotLight(GL_LIGHT1, 0, 5, 0, 0, 0, 0, 1, 1, 1);

	glPushMatrix();
	glTranslatef(xval, .5, zval);
	...
	glShadeModel(GL_FLAT);
	glPushMatrix();
	SetMaterial(204. / 255., 102. / 255., 1., 5);
	glTranslatef(-2, 0, 0);
	glRotatef(-90, 1, 0, 0);
	glutSolidCone(.5, 1.7, 30, 30);
	glPopMatrix();
	...
	SetMaterial(1, 0, 0., 2);
	glTranslatef(2, .5, 0);
		SetPointLight(GL_LIGHT2, 0, 0, 0, 1, 0, 0.);
		glPushMatrix();
			glColor3f(1, 0, 0.);
			glutSolidSphere(.01, 30, 30);
		glPopMatrix();
	...
	SetMaterial(0, 0, 1., 7.5);
	glPushMatrix();
	glTranslatef(0, .5, -2);
	glutSolidTorus(.3, .7, 30, 30);
	glPopMatrix();
